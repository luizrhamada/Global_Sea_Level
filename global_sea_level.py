...
- Nível do mar
Os níveis globais do mar estão subindo como resultado do aquecimento global causado pelo homem, com taxas recentes sem
precedentes nos últimos 2000 anos. O aumento do nível do mar é causado principalmente por dois fatores relacionados ao
aquecimento global: a água adicionada do derretimento de camadas de gelo e geleiras e a expansão da água do mar à medida
que ela esquenta. Este conjunto de dados contém a mudança no nível do mar desde 1993, conforme observado por satélites.

- Variáveis
Col1 - Ano

Col2 - Observações ponderadas totais

Col3 - GMSL (Global Isostatic Adjustment (GIA) not applied) - Variação do GMSL (Ajuste isostático global (GIA) não
aplicado) (mm) em relação à referência média colinear TOPEX/Jason de 20 anos
# É a variação sem a aplicação do ajuste isostático global a mudança do nível médio global. A média colinear TOPEX/Jason
de 20 anos refere-se ao valor médio do nível do mar calculado a partir de dados coletados ao longo de 20 anos pelos
satélites TOPEX/Poseidon e Jason. Esses satélites foram projetados para medir a altura da superfície do oceano com alta
precisão, utilizando altimetria por radar.

Col4 - Desvio padrão da estimativa de variação do GMSL (GIA não aplicado) (mm)

Col5 - Variação suavizada (filtro do tipo gaussiano de 60 dias) do GMSL (GIA não aplicado) (mm)
# A variação suavizada do ajuste isostático global refere-se à resposta lenta e gradual da crosta terrestre às mudanças
na carga superficial, como o derretimento de geleiras ou a erosão de montanhas. Esse processo é parte do ajuste
isostático, que é o mecanismo pelo qual a Terra busca restabelecer o equilíbrio gravitacional (isostasia) após
alterações na distribuição de massas na superfície.

Col6 - Variação do GMSL (Ajuste isostático global (GIA) aplicado) (mm) em relação à referência média colinear
# O Ajuste Isostático Global (GIA, do inglês Global Isostatic Adjustment) é um processo geofísico que descreve a
resposta lenta da Terra às mudanças na distribuição de cargas na superfície, como o derretimento de grandes massas de
gelo após as últimas glaciações. Esse fenômeno ocorre porque a crosta terrestre e o manto superior se ajustam ao longo
de milhares de anos para alcançar um novo equilíbrio isostático, à medida que o gelo derrete e a pressão sobre a
superfície diminui.

# Quando aplicado em relação à referência média colinear, o GIA é utilizado para corrigir medições de altimetria ou de
mudanças no nível do mar, levando em conta os efeitos do ajuste isostático. A referência média colinear é um sistema de
referência que considera a média global do nível do mar ao longo do tempo, ajustada para fatores como a subsidência ou
elevação da crosta terrestre causada pelo GIA.

# O GIA afeta as medições do nível do mar porque a crosta terrestre pode estar subindo ou descendo em diferentes regiões
devido ao ajuste isostático. Isso pode criar a ilusão de que o nível do mar está subindo ou descendo localmente, quando,
na verdade, é a crosta que está se movendo. O GIA é usado para corrigir essas medições em relação à referência média
colinear, garantindo que as mudanças reais no nível do mar sejam precisamente quantificadas.

# O GIA é crucial para entender as variações do nível do mar em escalas globais e regionais, especialmente ao separar os
efeitos do derretimento de gelo e da expansão térmica dos oceanos dos movimentos verticais da crosta terrestre.

Col7 - Desvio padrão da estimativa de variação do GMSL (GIA aplicado) (mm)

Col8 - Variação suavizada (filtro do tipo gaussiano de 60 dias) do GMSL (GIA aplicado) (mm)

Col9 - Suavizada (filtro do tipo gaussiano de 60 dias) Variação GMSL (GIA aplicada) (mm); sinal anual e semestral
removido
...
"""
Sea Level
Global sea levels are rising as a result of human-caused global warming, with recent rates being unprecedented over the 
past 2,000-plus years. Sea level rise is caused primarily by two factors related to global warming: the added water 
from melting ice sheets and glaciers and the expansion of seawater as it warms. This dataset contains the change in sea 
...
Col3 - GMSL (Global Isostatic Adjustment (GIA) not applied) variation (mm) with respect to 20-year TOPEX/Jason collinear 
mean reference
# It's no application of the global isostatic adjustment that the global mean sea level changes. The TOPEX/Jason 20-year 
collinear mean refers to the mean sea level value calculated from data collected over 20 years by the TOPEX/Poseidon and 
Jason satellites. These satellites were designed to measure the height of the ocean surface with high precision using 
radar altimetry.
...
Col6 - GMSL (Global Isostatic Adjustment (GIA) applied) variation (mm) with respect to 20-year TOPEX/Jason collinear 
mean reference
